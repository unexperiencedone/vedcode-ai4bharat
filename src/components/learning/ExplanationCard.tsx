...
export function ExplanationCard({
  ...
  // Reset proceed state when keyword changes
  useEffect(() => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      {showReadinessCheck ? (
        ...
        <>
          {/* ⚠️ Insight Phase 4: Gap Alert (Secondary reminder if proceeded) */}
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex flex-col items-end">
              ...
              <span className="text-[10px] text-muted-foreground/60 mt-1">
                Ebbinghaus tracking initializing...
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-2 flex flex-col gap-6">
              <div className="bg-card border border-border/50 rounded-xl overflow-hidden shadow-sm">
                <div className="border-b border-border/50 bg-muted/30 px-4 py-3 flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-foreground">
                    Code-First Context (Your Project)
                  </span>
                </div>
                  ...
                  {explanation ? (
                    ...
                    <>
                      <div className="text-muted-foreground text-xs mb-2">
                        📄 src/lib/schema.ts: L42-51
                      </div>
                      <pre>
                        <code className="text-blue-400">export const</code>{" "}
                        <span className="text-yellow-300">UserSchema</span> =
                        z.object({"{ \n"}
                        {"  "}email: z
                        ...
                        {"  "}password: z
                        <span className="text-green-300">.string()</span>
                        <span className="text-green-300">.min(</span>8
                        <span className="text-green-300">)</span>,{"\n"}
                        {"  "}role: z
                        <span className="text-green-300">.enum(</span>["USER",
                        "ADMIN"]<span className="text-green-300">)</span>,{"\n"}
                        {"}"})<span className="text-green-300">.refine</span>
                        ((data) =&gt; ...);
                      </pre>
                    </>
                  )}
                ...
                <div className="p-5 text-sm leading-relaxed text-muted-foreground">
                  ...
                  You are using `{keyword}` within the UserSchema to rigidly
                  define the shape of your authentication inputs before they
                  ever touch Drizzle ORM. This acts as a protective layer
                  preventing invalid emails or short passwords from causing
                  database errors.
                </div>
              </div>
              ...
              <div className="bg-card border border-border/50 rounded-xl overflow-hidden shadow-sm">
                ...
                <div className="p-5 text-sm leading-relaxed text-muted-foreground">
                  {keyword} is a schema declaration and data validation
                  methodology. Rather than manually writing `if (typeof data !==
                  'string')` everywhere, you define a single schema that
                  guarantees the exact types and formats. If data fails the
                  schema, it throws an error immediately rather than propagating
                  bad state through your application.
                </div>
              </div>
            </div>
      ...
      {/* 🔁 Insight Phase 4: Recall Prompter */}
      {mastery && mastery.recall < 0.45 && (
        <RecallPrompter
          conceptName={keyword}
          recallScore={mastery.recall}
          onStartChallenge={() => console.log("Challenge started")}
        />
      )}
    </div>
  );
