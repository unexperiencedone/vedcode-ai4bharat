...
export default function IngestionPanel({ embedded = false }: { embedded?: boolean }) {
  ...
  const startPolling = (sourceId: string) => {
    ...
    pollRef.current = setInterval(async () => {
      try {
        const res = await fetch(`/api/vedcode/ingest?sourceId=${sourceId}`);
        if (!res.ok) return;
  ...
  const handleIngest = async () => {
    ...
    try {
      const res = await fetch("/api/vedcode/ingest", {
        method: "POST",
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch("/api/vedcode/search", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="flex-1 h-full overflow-y-auto bg-background text-slate-100 font-sans p-8 selection:bg-blue-500/20">
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        <div className="space-y-2">
          ...
          <p className="text-slate-500 text-sm ml-[52px]">
            Feed documentation URLs into the VedCode vector store. Chunks are embedded with Titan 512d and stored in DynamoDB for semantic search.
          </p>
        </div>
