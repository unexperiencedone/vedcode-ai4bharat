...
export default function ConceptGalaxy() {
  ...
  const loadGalaxy = async (sourceId: string) => {
    ...
    try {
      const res = await fetch(
        `/api/vedacode/galaxy?sourceId=${encodeURIComponent(sourceId)}`,
      );
  ...
  const handleNodeClick = async (nd: ConceptNode) => {
    ...
    try {
      const res = await fetch(
        `/api/vedacode/reader?sourceId=${encodeURIComponent(activeMeta.sourceId)}`,
      );
      const json = await res.json();
      if (!res.ok) throw new Error(json.error);
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-500">
      ...
      <div className="relative flex gap-4">
        ...
        <div
          className={cn(
            "relative bg-[#020617] border border-slate-800/60 rounded-2xl overflow-hidden flex-shrink-0 transition-all duration-500",
