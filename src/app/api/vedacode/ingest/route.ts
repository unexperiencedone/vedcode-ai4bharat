/**
 * POST /api/vedacode/ingest  — kick off ingestion job
 * GET  /api/vedacode/ingest?sourceId=xxx — poll status
 */
import { NextResponse } from 'next/server';
import { ingestSource } from '@/lib/vedacode/ingestion';
...
export async function POST(req: Request) {
    try {
        ...
        return NextResponse.json({
            sourceId,
            status: 'started',
            message: `Ingestion started for ${sourceName}. Poll /api/vedacode/ingest?sourceId=${sourceId} for status.`,
        });
    } catch (error: any) {
