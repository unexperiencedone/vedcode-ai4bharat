...
export async function explainKeyword(keyword: string, contextChunks: string[]) {
  ...
  const { text } = await generateText({
    model: openai("gpt-4o"),
    system: "You are the JIT Explainer Agent for VedCode. Your job is to explain programming keywords and concepts specifically within the context of the user's codebase.",
