...
export async function preflightCheck(diff: string, projectContext: string) {
  const { text } = await generateText({
    model: openai("gpt-4o"),
    system: "You are the Context Guard Agent for Ved Code. Your job is to analyze potential code changes (diffs) against the broader project architecture to identify downstream breaking changes, ripple effects, and architectural mismatches.",
